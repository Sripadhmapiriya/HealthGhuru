...
const articleSchema = z.object({
  ...
  medicalReviewerName: z.string().optional(),
    ...
    try {
      await sql`
        INSERT INTO content_items (
          ...
          medical_reviewer_name, reviewed_at, requires_review, quality_score, relevance_score, created_at, updated_at
        ) VALUES (
          ${newId}::uuid, 'article', ${validated.title}, ${validated.slug}, ${validated.excerpt}, ${validated.excerpt},
          ${'/article/' + validated.slug}, ${validated.heroImageUrl || null}, ${validated.authorName || 'HealthGhuru Editorial Team'},
          CURRENT_TIMESTAMP, NULL, ${validated.status}, 'en',
          ${validated.category}, FALSE, ${validated.isFeatured ?? true}, ${validated.isTrending ?? false}, 
          ${validated.isBreaking ?? false}, ${validated.isEditorPick ?? false}, TRUE,
          ${validated.medicalReviewerName || null}, ${validated.medicalReviewerName ? sql`CURRENT_TIMESTAMP` : null},
        ...
        ON CONFLICT (slug) DO UPDATE SET
          ...
          medical_reviewer_name = EXCLUDED.medical_reviewer_name,
    ...
    try {
      await sql`
        ...
        SET
          ...
          ${validated.isBreaking !== undefined ? sql`is_breaking = ${validated.isBreaking},` : sql``}
          ${validated.isEditorPick !== undefined ? sql`is_editor_pick = ${validated.isEditorPick},` : sql``}
          ${validated.medicalReviewerName ? sql`medical_reviewer_name = ${validated.medicalReviewerName}, reviewed_at = CURRENT_TIMESTAMP,` : sql``}
